...
def dom_class_cell_1(
    ...
    with tf.variable_scope("domclass_1",reuse = tf.AUTO_REUSE):
       ...
       val,state = tf.nn.dynamic_rnn(cell_1,inputs=enc_output,dtype=tf.float32,sequence_length=enc_output_lengths)

       val = tf.transpose(val,[1,0,2])
       print(val.get_shape())
       last = tf.gather(val,tf.shape(val)[0] - 1)
       #print(last.get_shape())
       w = tf.get_variable("w",[num_hidden,2],initializer=tf.random_normal_initializer())

       b = tf.get_variable("b",[2],initializer=tf.constant_initializer(0.1))
...
def seq2seq_model(
    ...
    Returns:
        ...
        search
    '''
    #input_data1, input_data2, input_data3, input_data4 = input_data
    #input_lengths1, input_lengths2, input_lengths3, input_lengths4 = input_lengths
    #target_data1, target_data2, target_data3, target_data4 = target_data
    #target_lengths1, target_lengths2, target_lengths3, target_lengths4 = target_lengths
    ...
    #enc_output_d, _, enc_lengths_d = encoding_layer(
    #    input_lengths_d,
    #    input_data_d,
    #    keep_prob)

    #print(enc_output1_.get_shape())
    #print(enc_lengths1_.get_shape())
    ...
    #enc_output1 = lambda: enc_output1_
    #enc_output2 = lambda: enc_output2_

    #enc_lengths1 = lambda: enc_lengths1_
    #enc_lengths2 = lambda: enc_lengths2_
    ...
    #enc_output1 = tf.cond(training_mode,enc_output1_t1,enc_output1_t2)
    #enc_output2 = tf.cond(training_mode,enc_output2_t1,enc_output2_t2)
    #enc_output3 = tf.cond(training_mode,enc_output3_t1,enc_output3_t2)
    #enc_output4 = tf.cond(training_mode,enc_output4_t1,enc_output4_t2)


    #enc_lengths1 = lambda: enc_lengths_
    #enc_lengths2 = lambda: tf.slice(enc_lengths_,[0],[FLAGS.batch_size/2])
    #enc_lengths1 = tf.cond(training_mode,enc_lengths1_t1,enc_lengths1_t2)
    #enc_lengths2 = tf.cond(training_mode,enc_lengths2_t1,enc_lengths2_t2)
    #enc_lengths3 = tf.cond(training_mode,enc_lengths3_t1,enc_lengths3_t2)
    #enc_lengths4 = tf.cond(training_mode,enc_lengths4_t1,enc_lengths4_t2)
